...
<head>
    ...
    <style>
        * { font-family: 'Inter', sans-serif; }
        ...
        .btn-cari:hover { transform: translateY(-2px); box-shadow: 0 10px 25px rgba(249,115,22,0.4); }
...
<body class="bg-gray-950 text-white min-h-screen">
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 bg-gray-950/80 backdrop-blur-xl border-b border-white/5">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
            <a href="{{ route('beranda') }}" class="flex items-center gap-2.5">
                ...
                <span class="font-bold text-lg tracking-tight">SIGAP <span class="text-brand-400">BDG</span></span>
            </a>
            <a href="{{ route('lapor') }}" class="text-sm bg-brand-600 hover:bg-brand-500 text-white font-semibold px-4 py-2 rounded-lg transition flex items-center gap-1.5">
    ...
    <main class="max-w-2xl mx-auto px-4 sm:px-6 pt-28 pb-16">
        ...
        <form action="{{ route('proses.lacak') }}" method="POST" class="bg-gray-900 border border-white/5 rounded-2xl p-5 sm:p-6 mb-8">
            ...
            <p class="text-xs text-gray-500 mb-4">Contoh format: <span class="text-brand-400 font-mono font-semibold">SIGAP-A1B2C3</span></p>
            <div class="flex gap-3">
                <input
                    ...
                    class="flex-1 bg-gray-800 border border-gray-700 text-white placeholder-gray-600 rounded-xl px-4 py-3 text-sm font-mono uppercase focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition"
                    autocomplete="off"
                    spellcheck="false"
                >
                <button type="submit" class="btn-cari bg-brand-500 hover:bg-brand-600 text-white font-bold px-6 py-3 rounded-xl shadow-lg shadow-brand-500/20 flex items-center gap-2 whitespace-nowrap">
        ...
        @isset($kodeLacak)
            ...
            <div class="hasil-masuk space-y-5">
                ...
                <div class="bg-gray-900 border border-white/5 rounded-2xl overflow-hidden">
                    <div class="px-5 sm:px-6 pt-5 pb-4 border-b border-white/5 flex items-center justify-between">
                        ...
                        @php
                            $warnaStatus = match($dataLaporan->status) {
                                ...
                                'Proses'   => ['bg' => 'bg-brand-500/10',   'border' => 'border-brand-500/20',   'text' => 'text-brand-400',   'dot' => 'bg-brand-400'],
                    ...
                    <div class="px-5 sm:px-6 py-5">
                        <div class="flex gap-2 mb-6">
                            ...
                            <div class="flex-1">
                                <div class="h-1.5 rounded-full {{ $i <= $indeksAktif ? 'bg-gradient-to-r from-brand-400 to-brand-500' : 'bg-gray-800' }} mb-2 transition-all"></div>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-5">
                            ...
                            <div class="bg-gray-800/50 rounded-xl p-3">
                                ...
                                <div class="text-xs font-mono text-brand-400">{{ $dataLaporan->latitude }}</div>
                                <div class="text-xs font-mono text-brand-400">{{ $dataLaporan->longitude }}</div>
                            </div>
                        </div>
                ...
                <div class="bg-gray-900/50 border border-white/5 rounded-2xl px-5 py-4 flex items-center justify-between text-sm">
                    ...
                    <a href="{{ route('lapor') }}" class="text-brand-400 hover:text-brand-300 font-semibold transition flex items-center gap-1.5">
                        Buat laporan baru
            ...
            <div class="hasil-masuk bg-red-950/50 border border-red-500/20 rounded-2xl p-8 text-center">
                ...
                <p class="text-gray-500 text-xs mb-6">Pastikan kamu memasukkan kode dengan benar, termasuk awalan <span class="text-brand-400 font-semibold">SIGAP-</span> dan 6 karakter setelahnya.</p>
                <a href="{{ route('lapor') }}" class="inline-flex items-center gap-2 bg-brand-600/80 hover:bg-brand-600 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition">
                    Buat Laporan Baru
                </a>
            </div>
