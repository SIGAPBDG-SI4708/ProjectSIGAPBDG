...
@section('konten')
    ...
    <div class="space-y-6">
        ...
        @if(session('sukses'))
            <div
                class="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 p-4 rounded-xl text-xs flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div
                    ...
                    @if(Auth::user()->role === 'Super Admin')
                        <form action="{{ route('admin.leaderboard.bonus') }}" method="POST" class="space-y-4">
                            ...
                            <div>
                                <label for="id_daerah"
                                    class="block text-xs text-slate-600 dark:text-slate-400 font-semibold mb-2">
                                    Pilih Kecamatan
                                </label>
                            ...
                            <div>
                                <label for="poin"
                                    class="block text-xs text-slate-600 dark:text-slate-400 font-semibold mb-2">Jumlah Poin
                                    Bonus</label>
                                <input type="number" name="poin" id="poin" min="1" max="500" required placeholder="Contoh: 100"
                                    class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500 transition">
                                @error('poin')
                                    <p class="text-[10px] text-red-500 mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                <label for="deskripsi"
                                    class="block text-xs text-slate-600 dark:text-slate-400 font-semibold mb-2">Alasan
                                    Penghargaan / Deskripsi</label>
                                <textarea name="deskripsi" id="deskripsi" rows="3" required
                                    placeholder="Contoh: Apresiasi tanggap bencana banjir mandiri di kelurahan..."
                                    class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500 transition resize-none"></textarea>
                                @error('deskripsi')
                                    <p class="text-[10px] text-red-500 mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <button type="submit"
                                class="w-full bg-brand-600 hover:bg-brand-500 text-white font-bold text-xs py-2.5 rounded-xl transition shadow-md shadow-brand-500/10 active:scale-95 flex items-center justify-center gap-1.5">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @else
                        <div
                            class="bg-slate-50 dark:bg-slate-850 border border-slate-100 dark:border-slate-800 p-4 rounded-xl text-center">
                            <svg class="w-8 h-8 text-slate-400 dark:text-slate-600 mx-auto mb-2" fill="none"
            ...
            <div class="lg:col-span-2">
                <div
                    class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm flex flex-col h-full justify-between">
                    <div>
                        ...
                        <div class="overflow-x-auto rounded-xl border border-slate-100 dark:border-slate-800">
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr
                                        class="bg-slate-50 dark:bg-slate-850 text-slate-500 dark:text-slate-400 text-[10px] font-bold uppercase tracking-wider border-b border-slate-100 dark:border-slate-800">
                                ...
                                <tbody>
                                    @forelse($riwayatPoin as $riwayat)
                                        <tr
                                            class="border-b border-slate-50 dark:border-slate-850 text-xs hover:bg-slate-50/50 dark:hover:bg-slate-800/10 transition">
                                            <td
                                                class="px-4 py-3 text-[10px] font-mono text-slate-400 dark:text-slate-500 whitespace-nowrap">
                                            ...
                                            <td class="px-4 py-3 text-slate-500 dark:text-slate-400 max-w-[180px] truncate"
                                                title="{{ $riwayat->deskripsi }}">
                                                {{ $riwayat->deskripsi }}
                                            </td>
                                            <td
                                                class="px-4 py-3 text-right pr-4 font-bold font-mono text-brand-600 dark:text-brand-400">
                                                +{{ $riwayat->poin }}
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="5"
                                                class="px-4 py-6 text-center text-xs text-slate-400 dark:text-slate-600 italic">
