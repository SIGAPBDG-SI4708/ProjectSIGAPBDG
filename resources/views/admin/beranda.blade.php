...
@section('konten')
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div
                class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm lg:col-span-3">
                <h3 class="text-sm font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                    <svg class="w-5 h-5 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                    </svg>
                    Grafik Ulasan Warga (Rating)
                </h3>
                <div class="relative w-full h-64 flex justify-center">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            new Chart(ratingCtx, {
                type: 'bar',
                data: {
                    labels: {!! json_encode($chartRatingLabels) !!},
                    datasets: [{
                        label: 'Jumlah Penilaian',
                        data: {!! json_encode($chartRatingData) !!},
