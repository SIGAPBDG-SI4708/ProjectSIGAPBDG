...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
    ...
    <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            ...
            <div class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Total {{ $daftarLaporan->total() }} laporan ditemukan</div>
        </div>
    ...
    <div class="py-16 text-center">
        <div class="w-14 h-14 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <svg class="w-7 h-7 text-slate-400 dark:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
        </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
                    <th class="text-left text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider px-5 py-3.5">Tracking ID</th>
                    <th class="text-left text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider px-5 py-3.5">Daerah</th>
                    <th class="text-left text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider px-5 py-3.5 hidden md:table-cell">Koordinat</th>
                    <th class="text-left text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider px-5 py-3.5">Status</th>
                    <th class="text-left text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider px-5 py-3.5 hidden sm:table-cell">Dilaporkan</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                    ...
                    <td class="px-5 py-4">
                        ...
                        <div class="text-xs text-slate-400 dark:text-slate-600">{{ $laporan->daerah->tingkat ?? '' }}</div>
                    </td>
                    <td class="px-5 py-4 hidden md:table-cell">
                        <div class="font-mono text-xs text-slate-500 dark:text-slate-400">{{ number_format($laporan->latitude, 5) }}</div>
                        <div class="font-mono text-xs text-slate-400 dark:text-slate-600">{{ number_format($laporan->longitude, 5) }}</div>
                    </td>
                    <td class="px-5 py-4">
                    ...
                    <td class="px-5 py-4 hidden sm:table-cell">
                        <div class="text-xs text-slate-600 dark:text-slate-400">{{ $laporan->created_at->translatedFormat('d M Y') }}</div>
                        <div class="text-xs text-slate-400 dark:text-slate-600">{{ $laporan->created_at->translatedFormat('H:i') }} WIB</div>
                    </td>
                    <td class="px-5 py-4">
    ...
    <div class="px-5 py-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
        ...
        <div class="flex items-center gap-1.5">
            ...
            @else
                <a href="{{ $daftarLaporan->previousPageUrl() }}" class="px-3 py-1.5 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-white bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition">← Prev</a>
            @endif
            @if($daftarLaporan->hasMorePages())
                <a href="{{ $daftarLaporan->nextPageUrl() }}" class="px-3 py-1.5 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-white bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition">Next →</a>
            @else
