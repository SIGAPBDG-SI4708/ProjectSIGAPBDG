...
@section('konten')
    <div
        ...
        <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-800">
            ...
            <form action="{{ route('admin.laporan.indeks') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1 relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Tracking ID" class="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition-all dark:text-white" onblur="this.form.submit()">
                </div>
                
                <select name="status" class="py-2 px-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-brand-500 dark:text-white cursor-pointer" onchange="this.form.submit()">
                ...
                <select name="daerah" class="py-2 px-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-brand-500 dark:text-white cursor-pointer" onchange="this.form.submit()">
                    <option value="Semua" {{ request('daerah') === 'Semua' || !request()->has('daerah') ? 'selected' : '' }}>Semua Daerah</option>
                    @foreach($daftarDaerah as $d)
                        <option value="{{ $d->id }}" {{ request('daerah') == $d->id ? 'selected' : '' }}>{{ $d->nama_daerah }}</option>
                    @endforeach
                </select>
                @endif
