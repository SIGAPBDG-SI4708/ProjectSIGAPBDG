        ...
        <div
            ...
            <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <div>
                    ...
                    <div class="text-xs text-slate-400 mt-0.5">{{ $daftarPengajuan->total() }} pengajuan dari Admin Daerah
                    </div>
                </div>
            </div>
            ...
            @if($daftarPengajuan->isEmpty())
                <div class="py-16 text-center">
                    <div
                        class="w-14 h-14 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center mx-auto mb-4">
                    ...
                    <div class="text-slate-400 dark:text-slate-600 text-xs">Pengajuan dari Admin Daerah akan muncul di sini
                    </div>
                </div>
            @else
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
                                <th class="text-left text-xs text-slate-400 font-semibold uppercase tracking-wider px-5 py-3.5">
                                    Laporan</th>
                                <th
                                    class="text-left text-xs text-slate-400 font-semibold uppercase tracking-wider px-5 py-3.5 hidden md:table-cell">
                                    Daerah</th>
                                <th
                                    class="text-left text-xs text-slate-400 font-semibold uppercase tracking-wider px-5 py-3.5 hidden lg:table-cell">
                                    Diajukan Oleh</th>
                                <th
                                    class="text-right text-xs text-slate-400 font-semibold uppercase tracking-wider px-5 py-3.5">
                                    Nominal</th>
                                <th class="text-left text-xs text-slate-400 font-semibold uppercase tracking-wider px-5 py-3.5">
                                    Status</th>
                                <th
                                    class="text-left text-xs text-slate-400 font-semibold uppercase tracking-wider px-5 py-3.5 hidden sm:table-cell">
                                    Waktu</th>
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                                    ...
                                    <td class="px-5 py-4 hidden md:table-cell">
                                        <div class="text-sm text-slate-700 dark:text-slate-300 font-medium">
                                            {{ $pengajuan->laporanInfrastruktur->daerah->nama_daerah ?? '-' }}</div>
                                        <div class="text-xs text-slate-400 dark:text-slate-600">
                                            {{ $pengajuan->laporanInfrastruktur->daerah->tingkat ?? '' }}</div>
                                    </td>
                                    <td class="px-5 py-4 hidden lg:table-cell">
                                        <div class="text-sm text-slate-700 dark:text-slate-300">
                                            {{ $pengajuan->pengguna->nama ?? '-' }}</div>
                                        <div class="text-xs text-slate-400">{{ $pengajuan->pengguna->role ?? '' }}</div>
                                    </td>
                                    <td class="px-5 py-4 text-right">
                                        <div class="text-sm font-bold text-slate-800 dark:text-white">Rp
                                            {{ number_format($pengajuan->nominal_diajukan, 0, ',', '.') }}</div>
                                    </td>
                                    <td class="px-5 py-4">
                                        @if($pengajuan->status_approval === 'Menunggu')
                                            <span
                                                class="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 border border-amber-100 dark:border-amber-800/50">
                                                <span class="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse"></span>Menunggu
                                            </span>
                                        @elseif($pengajuan->status_approval === 'Disetujui')
                                            <span
                                                class="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800/50">
                                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>Disetujui
                                            </span>
                                        @else
                                            <span
                                                class="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-100 dark:border-red-800/50">
                                    ...
                                    <td class="px-5 py-4 hidden sm:table-cell">
                                        <div class="text-xs text-slate-500 dark:text-slate-400">
                                            {{ $pengajuan->waktu_pengajuan ? \Carbon\Carbon::parse($pengajuan->waktu_pengajuan)->translatedFormat('d M Y') : '-' }}
                                        </div>
                                        <div class="text-xs text-slate-400 dark:text-slate-600">
                                            {{ $pengajuan->waktu_pengajuan ? \Carbon\Carbon::parse($pengajuan->waktu_pengajuan)->translatedFormat('H:i') . ' WIB' : '' }}
                                        </div>
                                    </td>
                                    <td class="px-5 py-4">
                                        @if($pengajuan->status_approval === 'Menunggu')
                                            <div class="flex items-center gap-2">
                                                <form action="{{ route('admin.pengajuan.proses', $pengajuan->id) }}" method="POST">
                                                    ...
                                                    <button type="submit"
                                                        class="text-xs font-semibold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 border border-emerald-200 dark:border-emerald-800/50 px-3 py-1.5 rounded-xl transition whitespace-nowrap">✓
                                                        Setujui</button>
                                                </form>
                                                <form action="{{ route('admin.pengajuan.proses', $pengajuan->id) }}" method="POST">
                                                    @csrf
                                                    ...
                                                    <button type="submit"
                                                        class="text-xs font-semibold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:hover:bg-red-900/50 border border-red-200 dark:border-red-800/50 px-3 py-1.5 rounded-xl transition whitespace-nowrap">✗
                                                        Tolak</button>
                                                </form>
                                            </div>
                                        @else
                ...
                @if($daftarPengajuan->hasPages())
                    <div class="px-5 py-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        <div class="text-xs text-slate-400">Menampilkan
                            {{ $daftarPengajuan->firstItem() }}–{{ $daftarPengajuan->lastItem() }} dari
                            {{ $daftarPengajuan->total() }}</div>
                        <div class="flex items-center gap-1.5">
                            @if($daftarPengajuan->onFirstPage())
                                <span
                                    ...
                                    Prev</span>
                            @else
                                <a href="{{ $daftarPengajuan->previousPageUrl() }}"
                                    class="px-3 py-1.5 text-xs text-slate-500 hover:text-slate-700 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition">←
                                    Prev</a>
                            @endif
                            @if($daftarPengajuan->hasMorePages())
                                <a href="{{ $daftarPengajuan->nextPageUrl() }}"
                                    class="px-3 py-1.5 text-xs text-slate-500 hover:text-slate-700 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition">Next
                                    →</a>
                            @else
                                <span
